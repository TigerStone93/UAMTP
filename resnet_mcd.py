...
class CrossAttention(nn.Module):
    def __init__(self, query_dim=128, key_value_dim=30, hidden_dim=256):
...
class ResNet(nn.Module):
    def __init__(self):
        ...
        """
        self.dropout = nn.Dropout(p=0.1) # 0.05
    ...
    def forward(self, map_input_tensor, record_input_tensor):
        ...
        out = self.dropout(self.fully_connected_block_1(fused_input))

        out_acc = self.dropout(self.fully_connected_block_2_velocity(out))
        out_ang = self.dropout(self.fully_connected_block_2_yaw(out))

        out_acc_1 = self.dropout(self.fully_connected_block_3_velocity_1(out_acc))
        out_acc_2 = self.dropout(self.fully_connected_block_3_velocity_2(out_acc))
        out_acc_3 = self.dropout(self.fully_connected_block_3_velocity_3(out_acc))
        out_acc_4 = self.dropout(self.fully_connected_block_3_velocity_4(out_acc))
        out_ang_1 = self.dropout(self.fully_connected_block_3_yaw_1(out_ang))
        out_ang_2 = self.dropout(self.fully_connected_block_3_yaw_2(out_ang))
        out_ang_3 = self.dropout(self.fully_connected_block_3_yaw_3(out_ang))
        out_ang_4 = self.dropout(self.fully_connected_block_3_yaw_4(out_ang))
