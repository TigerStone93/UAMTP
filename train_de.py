...
def training_args():
    ...
    optimizer = "sgd"
    loss = "cross_entropy"
    weight_decay = 5e-4 # 10e-4 # 0.001 # 5e-4 # 0.0005 # higher value for bigger dataset and more complex model
    log_interval = 50
    pause_interval = 50
    save_interval = 50
    save_loc = "./"
...
if __name__ == "__main__":
    ensemble_size = 4
    for ensemble_model_index in range(ensemble_size):
        # Parsing the arguments
        args = training_args().parse_args()
        print("[Info] args :", args)

        print(f"[Info] ensemble_size : {ensemble_size}")
        print(f"[Info] ensemble_model_index : {ensemble_model_index}")

        # Setting the seed
        args.seed = ensemble_model_index
